...
function getDayName(date) {
  ...
  return week[dateFormat.getUTCDay()];
}
...
function getNextFriday(date) {
  ...
  let difference = (12 - weekDay) % 7;
  difference = difference === 0 ? 7 : difference;

  return new Date(currentDate.setUTCDate(currentDate.getDate() + difference));
}
...
function isDateInPeriod(date, period) {
  const dateFormatted = Number(new Date(date));
  const start = Number(new Date(period.start));
  const end = Number(new Date(period.end));

  if (dateFormatted >= start && dateFormatted <= end) return true;
  return false;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let day = 1; day <= daysCount; day += 1) {
    ...
    if (date.getDay() === 0 || date.getDay() === 6) weekendsCount += 1;
  }
...
function getWeekNumberByDate(date) {
  ...
  return Math.floor((Number(date) - Number(startYear)) / msPerWeek) + 1;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const startElements = period.start.split('-');
  const day = new Date(
    startElements[2],
    startElements[1] - 1,
    startElements[0]
  );
  const endElements = period.end.split('-');
  const end = new Date(endElements[2], endElements[1] - 1, endElements[0]);
  const result = [];
